...
def create_method_comparison_table(results_dict):
    ...
    for method, result in results_dict.items():
        ...
        try:
            # For ARIMAX, parameter names are different
            if 'ARIMAX' in result['method']:
                # ARIMAX stores exog variables as 'x1', 'x2', etc. in order
                params = model.params
                pvalues = model.pvalues
                bse = model.bse
                
                # Find indices for our variables
                # Order is: time, post, time_since (matching formula)
                imm_coef = params.iloc[2] if len(params) > 2 else np.nan  # post variable
                imm_pval = pvalues.iloc[2] if len(pvalues) > 2 else np.nan
                imm_se = bse.iloc[2] if len(bse) > 2 else np.nan
                
                trend_coef = params.iloc[3] if len(params) > 3 else np.nan  # time_since
                trend_pval = pvalues.iloc[3] if len(pvalues) > 3 else np.nan
                trend_se = bse.iloc[3] if len(bse) > 3 else np.nan
                
                rsquared = np.nan  # ARIMAX doesn't have R²
            else:
                imm_coef = model.params.get(post_var, np.nan)
                imm_pval = model.pvalues.get(post_var, np.nan)
                imm_se = model.bse.get(post_var, np.nan)
                
                trend_coef = model.params.get(time_var, np.nan)
                trend_pval = model.pvalues.get(time_var, np.nan)
                trend_se = model.bse.get(time_var, np.nan)
    ...
    for _, row in df_comparison.iterrows():
        print(f"\n{row['Method']}:")
        print(f"  Immediate Effect: {row['Immediate_Coef']:.4f} (SE: {row['Immediate_SE']:.4f}, p: {row['Immediate_p']:.4f})")
        sig = '***' if row['Immediate_p'] < 0.001 else '**' if row['Immediate_p'] < 0.01 else '*' if row['Immediate_p'] < 0.05 else 'ns'
        print(f"    Significance: {sig}")
        
        print(f"  Trend Change: {row['Trend_Coef']:.4f} (SE: {row['Trend_SE']:.4f}, p: {row['Trend_p']:.4f})")
    ...
    for interv_name, interv_info in INTERVENTIONS.items():
        ...
        if 'arimax' in method_results:
            ...
            plot_residual_diagnostics(
                ...
                save_path=f'diagnostics_arimax_{interv_name}.png'
            )
